...
const getForecast = (lat: number, lon: number): Promise<any> => {
  return WEATHER_API.get(
    `onecall?lat=${lat}&lon=${lon}&exclude=current,hourly,minutely,alerts&units=metric&&appid=${WEATHER_APP_ID}&units=metric`
  ).then((res) => res.data);
};
